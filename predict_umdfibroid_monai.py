...
def _cfg_get_tuple3_or_int(cfg: dict, key: str, default=None):
    """Read a MONAI constructor value that may be scalar or a 3-vector."""
...
def _factor_swin_window_from_bias_table(n: int) -> Optional[Tuple[int, int, int]]:
    """
    Infer Swin window_size from the first relative_position_bias_table length.

    MONAI stores tables with length:
        prod(2 * window_size[d] - 1)
    Common cases here are 7^3 -> 2197 and (4, 7, 7) -> 1183.
    """
    n = int(n)
    candidates = []
    for wz in range(2, 17):
        for wy in range(2, 17):
            for wx in range(2, 17):
                if (2 * wz - 1) * (2 * wy - 1) * (2 * wx - 1) == n:
                    candidates.append((wz, wy, wx))
    if not candidates:
        return None

    # Prefer the medical-imaging anisotropic convention z <= y == x, otherwise
    # choose the most isotropic candidate.
    candidates.sort(key=lambda t: (0 if (t[1] == t[2] and t[0] <= t[1]) else 1,
                                   abs(t[0] - t[1]) + abs(t[1] - t[2]),
                                   t))
...
def build_model(
    ...
    if name == "swinunetr":
        ...
        # Checkpoint-derived Swin constructor arguments must take priority over
        # prediction-time heuristics. If patch/window are rebuilt differently,
        # patch_embed and relative_position_* tensors will not match.
        explicit_patch_size = _cfg_get_tuple3_or_int(cfg, "swin_patch_size", default=None)
        explicit_window_size = _cfg_get_tuple3_or_int(cfg, "swin_window_size", default=None)

        patch_size: Any = explicit_patch_size if explicit_patch_size is not None else 2
        window_size: Any = explicit_window_size if explicit_window_size is not None else win

        if explicit_patch_size is None and explicit_window_size is None and swin_anisotropy_aware:
...
def load_models_from_checkpoints(
    ...
    for p in ckpts:
        ...
        if model_key == "swinunetr":
            inferred_swin = _infer_swin_params_from_state_dict(sd)
            if inferred_swin:
                cfg = dict(cfg)
                ...
                print(
                    ...
                    f"window_size={cfg.get('swin_window_size', None)}"
                )
...
def main():
    ...
    ap.add_argument(
        ...
        dest="swin_anisotropy_aware",
        action="store_true",
        default=True,
        help="If set (default), adjust Swin patch/window/roi to respect anisotropy + /32 constraints when checkpoint params cannot be inferred.",
    )
    ap.add_argument(
        "--no-swin-anisotropy-aware",
        dest="swin_anisotropy_aware",
        action="store_false",
        help="Disable the Swin anisotropy heuristic and use checkpoint/plans ROI directly.",
    )
